...
SDL_Surface * surf_menu_frames[FRAME_COUNT];
int loaded_frames = 0;
...
int MenuFrames(void *);
void FrameMenu(app_t *, SDL_Surface *);
...
app_t * WindowInitialization()
{
    ...
    else if(Mix_OpenAudio(48000, AUDIO_F32SYS, 2, 2048))
    {
    ...
    app->music = Mix_LoadMUS("./snd.mp3");
    if (app->music == NULL)
    {
        printf("Mixer could not be created! SDL_Error: %s\n", SDL_GetError());
        return (void*) 0;
    }
    return (app_t *) app;
}
void ScreenRenderingAndUpdating(app_t * app)
{
    ...
    SDL_Event event;
    char ld_txt[64];
    SDL_Color color_ld_text = {0xcc, 0xa2, 0x30};
    SDL_Surface * ld_bg = IMG_Load("bg.jpg");
    ...
    SDL_Surface * surf_menu = SDL_CreateRGBSurface(0, SCREEN_WIDTH, SCREEN_HEIGHT, 32, 0, 0, 0, 1);
    {
    ...
    SDL_CreateThread(MenuFrames, "menu_frames_loader", NULL);     
    while (state.flag)
    {
...
void FrameMenu(app_t * app, SDL_Surface * surf_menu_text)
{
    static char _no_music = TRUE;
    static int cn = 0;
    if (_no_music || (int) Mix_GetMusicPosition(app->music) == 31)
    {
    ...
    if (cn < loaded_frames)
    {
        SDL_BlitSurface( surf_menu_frames[cn], & (surf_menu_frames[cn]) -> clip_rect, app->surface, NULL);
        cn++;
    }
    SDL_BlitSurface(surf_menu_text, &surf_menu_text->clip_rect, app->surface, NULL);
    return;
}
void FrameGame(SDL_Surface * surface)
{
    DrawingGrid(GRID_COUNT, surface);
    return;
}
...
{
    char buffer[64];
    for (int i = 0; i < FRAME_COUNT; i++, loaded_frames++)
    {
        snprintf(buffer, sizeof(buffer), "./Game Mock-up/game_frames/%d.png", i + 1);
        surf_menu_frames[i] = IMG_Load(buffer);
        if (surf_menu_frames[i] == NULL)
        {
            printf("Faied at load frame in %s: %d!\n", i + 1);
            __asm__ volatile
            (
                "mov rax, 60\n"
                "mov rdi, 1\n"
