...
uint_16 loaded_frames_counter = 1;

int LoadingMenuFrames(void *)
{
    ...
    {
        if (InsertIntIntoString(current_frame, LENGTH_OF_ARRAY_path_to_img_frames_of_menu, loaded_frames_counter, path_to_img_frames_of_menu) == FALSE)
            return FALSE;
        
        array_of_menu_frames[loaded_frames_counter - 1] = IMG_Load((const char *) current_frame);
    }
